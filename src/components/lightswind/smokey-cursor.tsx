...
type SmokeyCursorColor = ColorRGB | string;
...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      if (!autoColors) {
        return normalizeColor(color, intensity);
      }
    ...
    function normalizeColor(input: SmokeyCursorColor, colorIntensity: number) {
      const rgb =
        typeof input === "string" ? parseColorString(input) : { ...input };
      const multiplier = 0.15 * colorIntensity;

      return {
        r: clamp01(rgb.r) * multiplier,
        g: clamp01(rgb.g) * multiplier,
        b: clamp01(rgb.b) * multiplier,
      };
    }

    function parseColorString(input: string): ColorRGB {
      const colorValue = input.trim();
  ...
  return (
    <div
      className={`fixed top-0 left-0 pointer-events-none w-full h-full cursor-none ${className}`}
      style={{ zIndex }}
    >
      <canvas
        ref={canvasRef}
